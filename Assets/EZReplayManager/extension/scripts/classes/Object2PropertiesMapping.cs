...
public class Object2PropertiesMapping : ISerializable{
	...
	public void insertStateAtPos(int recorderPosition) {
		...
		if (lastChangedFrame > -1) {
			...
			if (savedStates.ContainsKey(lastChangedFrame) && !newState.isDifferentTo(savedStates[lastChangedFrame], this)) {
		...
		if (firstChangedFrame > -1 && recorderPosition >= firstChangedFrame )  {
				if (recorderPosition <= lastChangedFrame ) {			
					...
					try {
						...
						getStateAtPos(recorderPosition).synchronizeProperties(gameObjectClone,this);
					...
					getStateAtPos(lastChangedFrame).synchronizeProperties(gameObjectClone,this); 
				}
