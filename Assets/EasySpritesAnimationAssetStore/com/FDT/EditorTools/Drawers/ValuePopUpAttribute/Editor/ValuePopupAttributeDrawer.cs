...
{
	[CustomPropertyDrawer(typeof(ValuePopupAttribute))]
	public class ValuePopupAttributeDrawer : PropertyDrawer
	{
		ValuePopupAttribute popupAttribute { get { return ((ValuePopupAttribute)attribute); } }
	    int index;

	    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	    {
	        // Checks to see what is the type of the provided values and acts accordingly.
			if ((System.Type)popupAttribute.variableType == typeof(int[]))
	        {
			...
			else if ((System.Type)popupAttribute.variableType == typeof(float[]))
	        {
	            EditorGUI.BeginChangeCheck();
	        ...
	        else if ((System.Type)popupAttribute.variableType == typeof(string[]))
	        {
	            EditorGUI.BeginChangeCheck();
	            // Checks all items in the provided list, to see if any of them is a match with the property value, if so assigns that value to the index.
	            for (int i = 0; i < popupAttribute.list.Length; i++)
