...
namespace com.FDT.EasySpritesAnimation.EditorInternal
{
	...
	public class SpriteAnimationAssetInspector : Editor
	{
		...
		public void DrawListItem (int animationIdx, Rect position, SerializedProperty property, SpriteAnimationData spriteAnimationData,  GUIContent label)
		{
			...
			object[] dragItems = EditorExtensions.DropZone<Sprite> ("Drag Sprites here", totalFrames);
			// test sprites
			if (!isDragginKeyframe && dragItems!=null &&dragItems.Length>0)
			{
				for (int i = 0; i < dragItems.Length; i++)
				{
					Sprite o = dragItems[i] as Sprite;
			...
			// 1
			newPos = new Rect (position.xMin, position.yMin + 5, position.width, 20);
			...
			newPos.width = 120;
			EditorGUI.LabelField (newPos, new GUIContent ("New Frames Time: "));
			newPos.xMin = newPos.xMax + 10;
			newPos.width = 60;
			EditorGUI.PropertyField (newPos, spNewFramesTime, new GUIContent (string.Empty));
			// 5
			...
			if (GUI.Button (newPos, "+")) 
			{
				...
				if (!spEditorConfirmations.boolValue || (spEditorConfirmations.boolValue && EditorUtility.DisplayDialog
				                                         ("New Keyframe",
				 "Are you sure you want to create a new animation keyframe ?", "Yes", "No")))
				{
					if ( spframeDatas.arraySize == 0)
					{
						spSelectedIdx.intValue = 0;
			...
			if (GUI.Button (newPos, "-")) 
			{
				if (!spEditorConfirmations.boolValue || (  spEditorConfirmations.boolValue && EditorUtility.DisplayDialog
				    ("Delete Keyframe ?",
				 "Are you sure you want to delete keyframe " + spSelectedIdx.intValue + " ?", "Yes", "No")))
				{
			...
			if (max>0)
			{
				...
				if (Event.current.type == EventType.Repaint) 
				{
					if (spriteProperty.objectReferenceValue != null) 
					{
						...
						Rect tr = spritePreview.textureRect;
		...
		protected void DrawBox(int idx, SerializedProperty property, SerializedProperty item, Rect pos, bool selected)
		{
			...
			Color selectedColor = (spriteProperty.objectReferenceValue != null) ? Color.cyan : Color.blue;
			Color unselectedColor = (spriteProperty.objectReferenceValue != null) ? Color.white : Color.grey;
